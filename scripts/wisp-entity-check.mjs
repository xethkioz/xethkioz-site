...
add('Home uses Wisp component through WorldStage', (worldStage.includes('<FusionWispEntity') || globalWisp.includes('<FusionWispEntity')) && home.includes('<FusionWorldStage') && !home.includes('className="green-wisp-secret"'), 'Home should use the reusable Wisp through the world-stage layer, not hardcode the Wisp markup.')
add('Wisp routes to Green Node', wisp.includes("'/green-node'") && (worldStage.includes('wispLabel') || globalWisp.includes('wispLabel') || globalWisp.includes('t.v7.core.wispLabel')), 'Wisp access must remain hidden route access to Green Node.')
