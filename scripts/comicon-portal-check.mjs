...
check('Published COMICON content loads from CMS', page.includes("fetchPublishedNews('comicon')") && page.includes('displayedArticles.map'))
check('Long COMICON content is split into focused views', page.includes("type ComiconView = 'overview' | 'comic' | 'archive' | 'news'") && page.includes("searchParams.get('view')") && page.includes("activeView === 'archive'") && page.includes("activeView === 'news'"))
...
check('Real catalog load records at least 40 sourced profiles', realCatalogCount >= 40 && realLoad.includes('Cada ficha incluye editorial/estudio') && realLoad.includes('imagen editorial original'))
...
check('Library renders real images or differentiated archive covers', library.includes('isGenericCatalogArtwork') && library.includes('<CatalogArtwork') && library.includes('data-variant={seed.variant}'))
check('Library keeps facts and official source links', library.includes('selectedItem.facts') && library.includes('selectedItem.source_urls'))
...
check('Archive covers include differentiated variants and channel identity', realLibraryCss.includes('[data-variant="5"]') && realLibraryCss.includes('[data-channel="marvel"]') && realLibraryCss.includes('.xk-comicon-catalog-art'))
