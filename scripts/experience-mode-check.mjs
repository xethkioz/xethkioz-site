...
const mode = read('src/lib/experienceMode.ts')
...
check('graphics mode persists safely', mode.includes("xethkioz.experience.graphics.v2") && context.includes('window.localStorage.setItem') && context.includes('catch'))
check('slow devices start in lite before React boots', main.includes('getInitialGraphicsMode()') && mode.includes("effectiveType === '3g'") && mode.includes('hardwareConcurrency'))
check('manual full or lite choice wins', mode.includes("stored === 'full' || stored === 'lite'") && mode.includes('return stored'))
...
check('lite avoids ambient video load', home.includes("useAmbientVideoEnabled(graphicsMode)") && mode.includes("graphicsMode === 'lite'") && home.includes('{videoEnabled && ('))
check('ambient video starts after idle instead of first interaction', home.includes('activationDelay') && home.includes('scheduleIdleTask') && !home.includes("window.addEventListener('pointerdown', activate)"))
...
check('science portal opens ArgenCiencia directly', home.includes("route: 'https://argenciencia.com/'") && home.includes('target="_blank"') && home.includes('noopener noreferrer'))
...
check('external science link is isolated', home.includes('noopener noreferrer') && science.includes('noopener noreferrer'))
