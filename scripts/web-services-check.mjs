...
check('Wisp remains hidden on mobile and clears the portal deck', !home.includes('<CompactWispPortal') && home.includes('hidden h-[300px] w-[250px]') && home.includes('xl:min-h-[320px]') && home.includes('xl:block'))
check('home universe exposes six proportional worlds without legacy arch frames', home.includes('<UniversePortalOrbit />') && universeOrbit.includes('xk-universe-core') && universeOrbit.includes('role="tablist"') && universeRegistry.includes("id: 'gaming'") && universeRegistry.includes("id: 'green'") && !universeOrbit.includes('rounded-t-[42%]'))
check('home portal atmosphere escapes card cuts and blends every world tone', universeOrbit.includes('xk-universe-cosmos') && redesignCss.includes('@keyframes xk-universe-turn') && redesignCss.includes('color-mix(in srgb,var(--universe-tone)') && redesignCss.includes('.xk-universe-core-image'))
