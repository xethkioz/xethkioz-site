...
check('Home uses the new optimized COMICON and Mascotas portal scenes', home.includes("world: '/assets/portal-comicon-duality-v11.webp'") && home.includes("world: '/assets/portal-mascotas-nature-v11-r2.webp'") && fs.existsSync(path.join(root, 'public/assets/portal-comicon-duality-v11.webp')) && fs.existsSync(path.join(root, 'public/assets/portal-mascotas-nature-v11-r2.webp')))
check('Home does not hide the new Mascotas scene behind a legacy override', !rootDocument.includes('huellas-portal-inline.css') && !fs.existsSync(path.join(root, 'public/huellas-portal-inline.css')) && !fs.existsSync(path.join(root, 'public/huellas-portal-image.js')))
check('Home gives the two new portals distinct lighting with reduced-motion support', home.includes("'data-portal': portal.id") && home.includes("portal.id === 'comicon' || portal.id === 'pets'") && read('src/pages/HomeReborn.css').includes('[data-portal="comicon"]') && read('src/pages/HomeReborn.css').includes('[data-portal="pets"]') && read('src/pages/HomeReborn.css').includes('xk-rb-portal-current') && read('src/pages/HomeReborn.css').includes('prefers-reduced-motion:reduce'))
