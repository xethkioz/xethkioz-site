...
check('Home exposes three public primary portals', !home.includes("id: 'comicon'") && home.includes('3 PORTALES PRINCIPALES ACTIVOS') && !district.includes("title: 'Green Node', detail: 'Archivos"))
...
check('Home uses the optimized Mascotas portal scene', home.includes("world: '/assets/portal-mascotas-nature-v11-r2.webp'") && fs.existsSync(path.join(root, 'public/assets/portal-mascotas-nature-v11-r2.webp')))
...
check('Home keeps the Mascotas lighting with reduced-motion support', home.includes("'data-portal': portal.id") && home.includes("portal.id === 'pets'") && !read('src/pages/HomeReborn.css').includes('[data-portal="comicon"]') && read('src/pages/HomeReborn.css').includes('[data-portal="pets"]') && read('src/pages/HomeReborn.css').includes('xk-rb-portal-current') && read('src/pages/HomeReborn.css').includes('prefers-reduced-motion:reduce'))
