...
func _show_title() -> void:
	...
	for i in range(labels.size()):
		var button := Button.new(); button.position=Vector2(70,290+i*52); button.size=Vector2(270,42); button.text=labels[i]; button.add_theme_font_size_override("font_size",14); _style_button(button); menu_layer.add_child(button)
		match i:
			0: button.pressed.connect(_new_game)
			1: button.pressed.connect(_continue_game)
			2: button.pressed.connect(_open_saved_overworld)
			3: button.pressed.connect(func(): _toast("AJUSTES • audio, resolución, idioma y accesibilidad se integran en la siguiente etapa de pulido.",4.0))
			4: button.pressed.connect(func(): _toast("WORLD OF XETHKIOZ • proyecto original XETHKIOZ.",3.0))
			5: button.pressed.connect(func(): get_tree().quit())
...
func _show_character_setup() -> void:
	...
	male.pressed.connect(func(): _choose_gender("Masculino")); female.pressed.connect(func(): _choose_gender("Femenino"))
...
func _choose_gender(value: String) -> void:
	...
	if traveler_preview_v10:
		...
		traveler_preview_v10.modulate = Color(1.0,0.94,1.0) if pending_gender == "Femenino" else Color.WHITE
...
func show_family_dialogue(speaker:String,text:String,relationship:String,gender:String) -> void:
	_ensure_dialogue_ui(); dialogue_speaker_v16=speaker; dialogue_layer.visible=true
	dialogue_name_label.text=speaker; dialogue_meta_label.text="%s • %s" % [relationship,gender]; dialogue_body_label.text=text; dialogue_timer=12.0
...
func _ensure_dialogue_ui() -> void:
	...
	dialogue_name_label=_vlabel(dialogue_layer,Vector2(205,518),Vector2(250,32),"",20,Color(1.0,0.72,0.32)); dialogue_meta_label=_vlabel(dialogue_layer,Vector2(460,522),Vector2(330,26),"",12,Color(0.68,0.70,0.80))
	...
	for i in range(4):
		var choice:=Button.new(); choice.name="Choice%d"%i; choice.position=Vector2(835,515+i*40); choice.size=Vector2(380,34); choice.add_theme_font_size_override("font_size",11); _style_button(choice); choice.pressed.connect(_dialogue_choice.bind(i)); dialogue_layer.add_child(choice)
...
func _update_dialogue_choices() -> void:
	...
	for i in range(4):
		var choice:Button=dialogue_layer.get_node("Choice%d"%i); choice.text=options[i]

func _dialogue_choice(index:int) -> void:
	match dialogue_speaker_v16:
		"Alexis":
			var alexis_answers=["Una grieta prismática abrió capas del mundo que nunca debieron tocarse.","Lo que conocíamos quedó mezclado con Izrdralar: ruinas del 2150, naturaleza y magia.","El colapso temporal fijó a los cuatro chicos en sus edades. El tiempo no corre igual alrededor de ellos.","Seguimos juntos. Eso es lo que importa ahora."]
			dialogue_body_label.text=alexis_answers[clampi(index,0,3)]
		"Ashley":
			var ashley_answers=["El bosque cambia de ritmo con cada clima. Si lo seguís, aparecen caminos que otros no ven.","Movimiento, combate y magia tienen cadencia. Aprenderla te hace más preciso.","Después del primer gran jefe vas a poder elegir una mentoría.","Dale. Izrdralar todavía tiene mucho para mostrar."]
			dialogue_body_label.text=ashley_answers[clampi(index,0,3)]
		_:
			dialogue_body_label.text="Cada respuesta abre otra parte del mundo."
	if index==3: dialogue_timer=2.0

func _start_level() -> void:
	if zone_intro_layer_v16: zone_intro_layer_v16.queue_free(); zone_intro_layer_v16=null
	super._start_level(); _show_zone_intro_v16()
...
func _show_zone_intro_v16() -> void:
	...
	var region:=_vlabel(panel,Vector2(10,7),Vector2(520,24),"%s — %s" % [WorldData.region_for_map(current_map),WorldData.biome_for_map(current_map)],14,Color(0.96,0.90,1.0)); region.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
	var map_name:=_vlabel(panel,Vector2(10,32),Vector2(520,20),"%02d • %s" % [current_map,WorldData.map_name(current_map)],11,Color(0.82,0.86,0.96)); map_name.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
	var tween:=create_tween(); tween.tween_interval(3.2); tween.tween_property(panel,"modulate",Color(1,1,1,0),0.5); tween.finished.connect(_free_zone_intro_v16)

func _free_zone_intro_v16() -> void:
	if zone_intro_layer_v16 and is_instance_valid(zone_intro_layer_v16): zone_intro_layer_v16.queue_free()
	zone_intro_layer_v16=null

func _complete_map() -> void:
	if map_complete: return
	map_complete=true; transition_finished_map=current_map; _mark_node_completed(transition_finished_map)
	...
	transition_next_map=transition_finished_map+1; current_map=transition_next_map; state["current_map"]=current_map; state["current_node"]=current_map
	supplies=min(float(state.get("max_supplies",100.0)),supplies+8.0); state["supplies"]=supplies; SaveSystem.save_state(state); _show_map_transition_v16()
...
func _show_map_transition_v16() -> void:
	...
	var can_refuge:bool=transition_finished_map%5==0; refuge.disabled=not can_refuge; refuge.tooltip_text="Disponible al cerrar cada bloque de 5 mapas." if not can_refuge else "Descansar, alquimia, mascotas, equipo, atlas y consejo."
...
func _transition_continue() -> void:
	if transition_finished_map==5 and not bool(state.get("mentor_chosen",false)): _show_mentor_select()
	else: _show_overworld()
...
func _show_elida_refuge_v16() -> void:
	...
	var refuge_menu:=Panel.new(); refuge_menu.position=Vector2(875,125); refuge_menu.size=Vector2(330,470); refuge_menu.add_theme_stylebox_override("panel",_panel_style(Color(0.015,0.018,0.035,0.96),Color(0.48,0.30,0.72,0.96),2)); menu_layer.add_child(refuge_menu)
	...
	for i in range(opts.size()):
		var option:=Button.new(); option.position=Vector2(895,145+i*52); option.size=Vector2(290,42); option.text=opts[i]; option.add_theme_font_size_override("font_size",11); _style_button(option); menu_layer.add_child(option)
		match i:
			0: option.pressed.connect(func(): _toast("Elida: Acá siempre van a tener un hogar. No dejes que la Fisura te haga olvidar por qué seguís caminando.",5.0))
			1: option.pressed.connect(_refuge_rest_v16)
			2: option.pressed.connect(_refuge_alchemy_v16)
			3: option.pressed.connect(_refuge_pet_v16)
			4: option.pressed.connect(_show_armory)
			5: option.pressed.connect(_show_overworld)
			6: option.pressed.connect(func(): _toast("Alexis: Si una ruta parece demasiado obvia, mirá arriba y abajo. Izrdralar siempre deja otra opción.",4.0))
			7: option.pressed.connect(_refuge_continue_v16)
...
func _refuge_alchemy_v16() -> void:
	...
	if crystals<5: _toast("ALQUIMIA • necesitás 5 cristales para preparar una ración prismática.",2.8); return
...
func _refuge_pet_v16() -> void:
	...
	if owned.is_empty(): _toast("MASCOTAS • todavía no desbloqueaste un legendario adicional.",2.8); return
