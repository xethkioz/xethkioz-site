...
check('Huellas endpoint uses the server-only service key', statsApi.includes('SUPABASE_SERVICE_ROLE_KEY') && !statsClient.includes('SUPABASE_KEY'))
check('Huellas RPCs are service-role only after rollout', cleanup.includes('revoke all on function public.get_huellas_stats() from public, anon, authenticated') && migration.includes('grant execute on function public.register_huellas_visit(uuid, inet) to service_role'))
