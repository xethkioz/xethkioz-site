...
const mascotas = read('src/pages/MascotasPortal.tsx')
...
add('Retired City routes lead to the community, never Huellas', !app.includes("import('./pages/FunPortal')") && app.includes('<Route path="/fun" element={<Navigate to="/community" replace />} />') && app.includes('path="/nexus-city/*"'), 'Preserve a safe community destination without restoring the retired world or changing Mascotas.')
add('Huellas remains an isolated static document', vercel.includes('{ "source": "/mascotas", "destination": "/mascotas/index.html" }') && vercel.includes('{ "source": "/mascotas/", "destination": "/mascotas/index.html" }') && !app.includes('<Route path="/mascotas"'), 'Both Huellas URL forms must resolve to the static microsite instead of the React application.')
add('Huellas exposes focused animal-service sections', ['Perdidos', 'Encontrados', 'Adopciones', 'Castraciones', 'Fauna y flora'].every((label) => mascotas.includes(label)) && mascotas.includes('/mascotas/publicar'), 'The replacement community portal must expose its real animal-service destinations.')
add('Huellas navigation remains usable on mobile', mascotas.includes('overflow-x-auto') && mascotas.includes('whitespace-nowrap') && mascotas.includes('md:px-6'), 'Animal-service navigation should scroll safely on small screens instead of overlapping the page.')
...
add('Green district cards open mounted sector views', ['?view=dossiers#archive', '?view=terminal#terminal', '?view=signals#evidence'].every((target) => nexusDistrict.includes(target)), 'Archive, Terminal and Evidence cards must select their view before scrolling to its anchor.')
add('Legacy Green sector hashes remain functional', green.includes("'#archive': 'dossiers'") && green.includes("'#terminal': 'terminal'") && green.includes("'#evidence': 'signals'") && green.includes('target.scrollIntoView'), 'Bare hash URLs must resolve a view and focus the mounted section.')
...
add('WISP preserves direct sector navigation', greenGuide.includes("item.view === activeView") && !greenGuide.includes("onNavigateRef.current('overview')"), 'Contextual help must not send a direct Archive, Terminal or Evidence visit back to the overview.')
...
add('Gaming hub is split into four focused sections', ['overview', 'guides', 'news', 'community'].every((section) => gaming.includes(`${section}:`)) && gaming.includes('xk-gaming-section-nav') && !gaming.includes("id: 'live'"), 'Gaming should expose only Start, Guides, Radar and Community.')
