...
add('Fun defaults to Huellas while preserving explicit Nexus access', fun.includes("const MASCOTAS_URL = 'https://www.xethkioz.com.ar/mascotas/'") && fun.includes('return <MascotasRedirect />') && fun.includes("location.hash === '#nexus-city'") && fun.includes("get('mode') === 'play'"), 'The retired meme gateway must lead to Huellas while keeping the legacy Nexus entrance intentional.')
add('Nexus City is lazy and mounted only on explicit request', fun.includes("const NexusCity = lazy(() => import('./NexusCity'))") && fun.includes('if (isNexusAccess)') && fun.includes('<NexusCity />'), 'The full Nexus hub should not mount during the default Huellas transition.')
