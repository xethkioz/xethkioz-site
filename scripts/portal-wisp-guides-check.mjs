...
add('Step navigation changes visible Fun content', component.includes('onNavigate(t.steps[boundedIndex].destination)') && fun.includes('onNavigate={navigateWithLumina}'), 'A Lumina guide step must mount the Fun section it explains.')
...
add('Gaming route state remains URL-backed without a duplicate guide', gaming.includes('selectSection') && gaming.includes('setSearchParams(next, { replace: true })') && !gaming.includes('<PortalWispGuide'), 'Gaming must keep shareable section navigation without restoring the duplicate guide removed from the streamlined page.')
