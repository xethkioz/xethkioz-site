...
func _run() -> void:
	...
	_event_bus.get("npc_interacted").emit("val")
	await process_frame
	...
	# Santuario y Boss 5.
	_event_bus.get("enemy_defeated").emit("custodio_raices_menor", 0, Vector2.ZERO)
	await process_frame
	...
	_event_bus.get("enemy_defeated").emit("boss5_guardian_bosque_velado", 0, Vector2.ZERO)
	await process_frame
	_require(_quest_state() == 8, "Boss 5 defeat must send player to Elida")

	# Refugio, mentoría inicial y cuarta pieza.
	_event_bus.get("npc_interacted").emit("elida")
	await process_frame
	_require(_quest_state() == 9, "Elida must open mentor choice")
	_event_bus.get("npc_interacted").emit("ashley")
	await process_frame
	...
	_event_bus.get("npc_interacted").emit("fermin")
	await process_frame
	_require(_quest_state() == 11, "Fermín must start Impact training")
	for _i in range(3):
		_event_bus.get("enemy_defeated").emit("nucleo_entrenamiento_impacto", 0, Vector2.ZERO)
		await process_frame
