...
type ServerEvents = {
  'nexus:history': (payload: { messages?: NexusMessage[] }) => void
  'nexus:message': (message: NexusMessage) => void
  'nexus:error': (payload: { message?: string }) => void
}

type ClientEvents = {
  'nexus:join': (payload: { room: string; nickname: string }) => void
  'nexus:message': (payload: { room: string; nickname: string; text: string }) => void
}
...
export default function NexusChatWidget() {
  ...
  useEffect(() => {
    ...
    const socket: Socket<ServerEvents, ClientEvents> = io(CHAT_URL, { transports: ['websocket', 'polling'], reconnectionAttempts: 5, auth: { room, nickname: cleanNickname(nickname) } })
    socketRef.current = socket
    socket.on('connect', () => { setStatus('online'); socket.emit('nexus:join', { room, nickname: cleanNickname(nickname) }) })
    socket.on('disconnect', () => setStatus('offline'))
    socket.on('connect_error', () => setStatus('error'))
    socket.on('nexus:history', (payload) => setMessages(payload.messages?.length ? payload.messages : [systemMessage('Sala lista.', room)]))
    socket.on('nexus:message', (message) => setMessages((current) => [...current.slice(-79), message]))
    socket.on('nexus:error', (payload) => setMessages((current) => [...current.slice(-79), systemMessage(payload.message || 'Error temporal.', room)]))
      ...
      {open && (
        <section className="mb-4 w-[calc(100vw-2.5rem)] max-w-[390px] overflow-hidden rounded-3xl border border-[#8B5CF6]/60 bg-[#0A0A0F]/95 shadow-[0_0_10px_#8B5CF6,0_18px_80px_rgba(0,0,0,0.65)] backdrop-blur-xl" aria-label="XETHKIOZ Nexus Chat">
          <header className="border-b border-[#8B5CF6]/30 bg-gradient-to-r from-[#8B5CF6]/20 via-black/40 to-[#FF6B1A]/15 p-4">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.32em] text-[#FF6B1A]">XETHKIOZ</p><h2 className="mt-1 text-lg font-black uppercase tracking-[0.18em] text-white">Nexus Chat</h2></div><span className="rounded-full border border-[#8B5CF6]/30 px-2 py-1 text-[10px] uppercase text-gray-300">{status}</span></div>
            <div className="mt-4 grid grid-cols-[1fr_auto] gap-2"><input value={nickname} onChange={(event) => setNickname(event.target.value)} className="rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-xs text-white outline-none focus:border-[#8B5CF6]" placeholder="Apodo" /><select value={room} onChange={(event) => { setRoom(event.target.value); setMessages([systemMessage(`Sala ${event.target.value}.`, event.target.value)]) }} className="rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-xs text-white outline-none focus:border-[#FF6B1A]">{rooms.map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}</select></div>
          </header>
