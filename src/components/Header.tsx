...
export default function Header() {
  ...
  return (
    <>
      <aside className="xk-left-launcher fixed left-4 top-1/2 z-[72] hidden -translate-y-1/2 flex-col gap-3 rounded-[2rem] border border-white/10 bg-black/45 p-2 shadow-[0_0_34px_rgba(139,92,246,.22)] backdrop-blur-xl md:flex" aria-label="XETHKIOZ launcher">
        {launcher.map((item) => (
          <Link key={item.to} to={item.to} className="grid h-12 w-12 place-items-center rounded-2xl border border-white/10 bg-white/[0.035] text-lg text-white transition hover:-translate-y-0.5 hover:border-[#8B5CF6] hover:shadow-[0_0_18px_rgba(139,92,246,.55)]" title={item.label} aria-label={item.label}>
            {item.icon}
          </Link>
        ))}
