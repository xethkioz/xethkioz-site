...
export default function ArticleCard({ article, variant = 'default' }: ArticleCardProps) {
  const { t } = useLang()
  const img = article.cover_image || 'https://images.pexels.com/photos/1545790948/pexels-photo-1545790948.jpeg?auto=compress&cs=tinysrgb&w=800'
  ...
  if (variant === 'hero') return (
    <Link to={`/article/${article.slug}`} className="group relative block overflow-hidden rounded-2xl news-card card-hover h-full min-h-[400px]">
      <div className="absolute inset-0"><img src={img} alt={article.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" /></div>
  ...
  if (variant === 'featured') return (
    <Link to={`/article/${article.slug}`} className="group relative block overflow-hidden rounded-2xl news-card card-hover">
      <div className="aspect-[16/10] overflow-hidden"><img src={img} alt={article.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy" /></div>
  ...
  if (variant === 'compact') return (
    <Link to={`/article/${article.slug}`} className="group flex gap-3 items-start">
      <div className="w-20 h-20 flex-shrink-0 overflow-hidden rounded-lg"><img src={img} alt={article.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" /></div>
  ...
  if (variant === 'horizontal') return (
    <Link to={`/article/${article.slug}`} className="group flex flex-col sm:flex-row gap-4 overflow-hidden rounded-xl news-card-purple card-hover">
      <div className="sm:w-48 aspect-[16/10] sm:aspect-auto overflow-hidden flex-shrink-0"><img src={img} alt={article.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" /></div>
  ...
  return (
    <Link to={`/article/${article.slug}`} className="group flex flex-col overflow-hidden rounded-xl news-card card-hover h-full">
      <div className="aspect-[16/10] overflow-hidden relative"><img src={img} alt={article.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" />{article.is_featured && <span className="absolute top-3 left-3 px-2 py-0.5 text-xs font-bold text-ink bg-orange rounded">⭐</span>}</div>
