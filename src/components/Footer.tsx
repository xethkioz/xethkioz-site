    ...
    <footer className="relative mt-20 border-t border-white/10 bg-ink-300">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-12">
          <div><Logo size="md" /><p className="mt-4 text-sm text-gray-400 leading-relaxed">{t.footer.tagline}</p><div className="mt-4 flex flex-wrap gap-2">{socials.map((s) => <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 text-xs font-medium text-gray-300 border border-white/10 rounded-md hover:border-orange hover:text-orange transition-all">{s.name}</a>)}</div></div>
