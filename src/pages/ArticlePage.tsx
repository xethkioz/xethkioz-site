...
export default function ArticlePage() {
  ...
  return (
    <div className="animate-fade-in">
      <SEO title={article.title} description={article.excerpt || ''} image={article.cover_image || undefined} type="article" publishedTime={article.published_at} author={article.author?.name} tags={article.tags} />
      <article className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center gap-4 mb-8 text-sm text-gray-500 border-y border-white/10 py-4">
          {article.author?.avatar_url && <img src={article.author.avatar_url} alt={article.author.name} className="w-10 h-10 rounded-full object-cover" />}
        ...
        {article.cover_image && <div className="rounded-2xl overflow-hidden mb-8"><img src={article.cover_image} alt={article.title} className="w-full h-auto object-cover" /></div>}
