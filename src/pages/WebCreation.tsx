...
export default function WebCreation() {
  ...
  function chooseOffer(offer: WebServiceOffer) {
    setForm((current) => ({
      ...current,
      serviceId: offer.id,
      projectType: projectTypeByOfferSlug[offer.slug] ?? current.projectType,
    }))
    setQuoteStep(1)
    ...
    <div className="min-h-screen overflow-hidden bg-[#07070c] text-white">
          ...
          {t.trust.map(([title, description], index) => (
            <article key={title} className="rounded-[1.4rem] border border-white/[0.07] bg-white/[0.035] p-5">
              <div className="flex items-start gap-4">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-orange-300/30 bg-orange-400/10 font-mono text-xs font-black text-orange-200" aria-hidden="true">0{index + 1}</span>
                <div>
                  <h2 className="font-mono text-xs font-black uppercase tracking-[0.15em] text-white">{title}</h2>
      ...
      <section id="presupuesto" className="scroll-mt-28 px-5 py-24 md:px-10 lg:px-14">
        <div className="mx-auto grid max-w-[1500px] gap-12 xl:grid-cols-[0.72fr_1.28fr]">
          ...
          <form ref={quoteFormRef} onSubmit={submitQuote} className="rounded-[2rem] border border-white/12 bg-gradient-to-br from-purple-500/[0.08] via-black/40 to-orange-500/[0.055] p-6 shadow-[0_35px_100px_rgba(0,0,0,.45)] md:p-9">
            <div className="border-b border-white/10 pb-6">
              <p className="font-mono text-[10px] font-black uppercase tracking-[0.2em] text-white/45">{t.quoteStep} {quoteStep} / 2</p>
              <ol className="mt-4 grid grid-cols-2 gap-3" aria-label={lang === 'es' ? 'Progreso de la solicitud' : 'Request progress'}>
                {t.quoteSteps.map((label, index) => {
                  const step = (index + 1) as QuoteStep
                  const active = quoteStep === step
                  const completed = quoteStep > step
                  return (
                    <li key={label} aria-current={active ? 'step' : undefined} className={`flex items-center gap-3 rounded-2xl border px-4 py-3 text-xs font-black uppercase tracking-[0.12em] ${active ? 'border-orange-300/45 bg-orange-400/10 text-orange-100' : completed ? 'border-purple-300/30 bg-purple-400/10 text-purple-100' : 'border-white/10 bg-black/20 text-white/35'}`}>
                      <span className={`grid h-7 w-7 place-items-center rounded-full font-mono text-[10px] ${active ? 'bg-orange-300 text-black' : completed ? 'bg-purple-400 text-black' : 'bg-white/10 text-white/50'}`}>{completed ? '✓' : step}</span>
                      {label}
                    </li>
                  )
                })}
            ...
            {submitState.status === 'success' ? (
              <div role="status" className="py-10 text-center">
                <span className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-orange-300/40 bg-orange-400/10 text-2xl text-orange-200" aria-hidden="true">✓</span>
                <h3 className="mt-6 text-3xl font-black tracking-[-0.025em]">{t.successTitle}</h3>
                <p className="mx-auto mt-4 max-w-lg text-sm leading-7 text-white/65">{t.success}</p>
                <button type="button" onClick={resetQuote} className="mt-7 rounded-full border border-purple-400/45 bg-purple-500/10 px-6 py-3 font-mono text-xs font-black uppercase tracking-[0.16em] text-purple-100 transition hover:border-orange-300 hover:text-orange-100">{t.newRequest}</button>
              </div>
            ) : (
              <>
                <div className="pt-7">
                  <h3 ref={stepHeadingRef} tabIndex={-1} className="text-2xl font-black tracking-[-0.02em] outline-none md:text-3xl">{quoteStep === 1 ? t.projectStepTitle : t.contactStepTitle}</h3>
                  <p className="mt-3 text-sm leading-6 text-white/55">{quoteStep === 1 ? t.projectStepText : t.contactStepText}</p>
                </div>
                ...
                {quoteStep === 1 ? (
                  <div className="mt-7 grid gap-5 md:grid-cols-2">
                    ...
                    {selectedOffer ? (
                      <div className="flex items-center gap-4 rounded-2xl border border-orange-300/20 bg-orange-400/[0.055] p-4 md:col-span-2">
                        <SafeImage src={selectedOffer.image_url} fallback="/web-services/landing-premium.svg" alt="" className="h-16 w-24 shrink-0 rounded-xl object-cover" />
                        <div className="min-w-0 flex-1">
                          <p className="font-mono text-[9px] font-black uppercase tracking-[0.16em] text-orange-200">{t.selectedOffer}</p>
                          <p className="mt-1 truncate text-sm font-black text-white">{selectedOffer.title}</p>
                        </div>
                        <button type="button" onClick={() => scrollTo('propuestas')} className="shrink-0 text-[10px] font-black uppercase tracking-[0.12em] text-purple-200 underline decoration-purple-300/40 underline-offset-4">{t.changeOffer}</button>
                      </div>
                    ) : null}
                    ...
                    <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-purple-100 md:col-span-2">
                      <span className="flex items-end justify-between gap-4"><span>{t.details}</span><span className="font-mono text-[9px] font-medium normal-case tracking-normal text-white/35">{form.details.length} / 2000 {t.characters}</span></span>
                  ...
                  <div className="mt-7 grid gap-5 md:grid-cols-2">
                    ...
                    <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-purple-100">
                      {t.whatsapp}
                      <input required={form.contactPreference === 'whatsapp'} type="tel" maxLength={40} autoComplete="tel" value={form.whatsapp} onChange={(event) => updateForm('whatsapp', event.target.value)} className="min-h-12 rounded-2xl border border-white/12 bg-[#0d0b14] px-4 py-3 text-sm font-medium normal-case tracking-normal text-white outline-none transition placeholder:text-white/25 focus:border-orange-300" placeholder="+54 9 …" />
                    </label>
                    <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-purple-100">
                      {t.business}
                    ...
                    <label className="flex items-start gap-3 text-sm leading-6 text-white/65 md:col-span-2">
                ...
                {quoteStep === 1 ? (
                  <button type="button" onClick={continueQuote} className="mt-7 w-full rounded-full bg-gradient-to-r from-orange-500 to-orange-300 px-7 py-4 font-mono text-xs font-black uppercase tracking-[0.16em] text-black shadow-[0_0_34px_rgba(255,106,0,.25)] transition hover:scale-[1.01] hover:shadow-[0_0_50px_rgba(255,106,0,.42)]">{t.nextStep} →</button>
                ) : (
                  <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row">
                    <button type="button" onClick={() => focusStep(1)} className="rounded-full border border-purple-400/40 bg-purple-500/[0.07] px-6 py-4 font-mono text-xs font-black uppercase tracking-[0.14em] text-purple-100 transition hover:border-purple-300 sm:w-auto">← {t.previousStep}</button>
      ...
      <section className="border-t border-white/10 bg-white/[0.02] px-5 py-24 md:px-10 lg:px-14">
        <div className="mx-auto grid max-w-[1500px] gap-12 xl:grid-cols-[0.7fr_1.3fr]">
          <div>
            <p className="font-mono text-xs font-black uppercase tracking-[0.3em] text-orange-300">{t.faqEyebrow}</p>
            <h2 className="mt-4 max-w-xl text-4xl font-black tracking-[-0.035em] md:text-6xl">{t.faqTitle}</h2>
          </div>
          <div className="space-y-3">
            {t.faqs.map(([question, answer], index) => (
              <details key={question} className="group rounded-[1.5rem] border border-white/10 bg-black/25 open:border-purple-400/35 open:bg-purple-500/[0.055]">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-5 px-5 py-5 text-left text-base font-black marker:hidden md:px-6">
                  <span className="flex items-center gap-4"><span className="font-mono text-[10px] text-orange-300" aria-hidden="true">0{index + 1}</span>{question}</span>
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-white/10 text-purple-200 transition group-open:rotate-45 group-open:border-orange-300/40 group-open:text-orange-200" aria-hidden="true">+</span>
                </summary>
                <p className="px-5 pb-6 pr-14 text-sm leading-7 text-white/60 md:px-6 md:pr-20">{answer}</p>
              </details>
            ))}
          </div>
        </div>
