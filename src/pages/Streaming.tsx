  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      {loading ? (
        ...
        <>
          {liveStreams.length > 0 && <section className="mb-12"><h2 className="section-title text-red-500 mb-6 flex items-center gap-2"><span className="w-3 h-3 bg-red-500 rounded-full animate-pulse" />{t.streaming.live}</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{liveStreams.map((s) => <a key={s.id} href={s.channel_url} target="_blank" rel="noopener noreferrer" className="group relative overflow-hidden rounded-xl news-card border-red-500/30 card-hover"><div className="aspect-video overflow-hidden"><img src={s.thumbnail || 'https://images.pexels.com/photos/3165335/pexels-photo-3165335.jpeg?auto=compress&cs=tinysrgb&w=400'} alt={s.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" /></div><span className="absolute top-3 left-3 px-2 py-1 text-xs font-bold text-white bg-red-600 rounded animate-pulse">{t.streaming.live}</span><div className="p-4"><h3 className="font-display text-sm font-bold text-white group-hover:text-orange transition-colors">{s.title}</h3><p className="text-xs text-gray-500 mt-1">{s.channel_name}</p></div></a>)}</div></section>}
          {platforms.map((p) => { const ps = streams.filter((s) => s.platform === p); if (!ps.length) return null; return (
            <section key={p} className="mb-12"><h2 className="section-title mb-6 capitalize"><span className={`inline-block w-2 h-6 ${pc[p]} rounded mr-2 align-middle`} />{p}</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{ps.map((s) => <a key={s.id} href={s.channel_url} target="_blank" rel="noopener noreferrer" className="group relative overflow-hidden rounded-xl news-card card-hover"><div className="aspect-video overflow-hidden"><img src={s.thumbnail || 'https://images.pexels.com/photos/3165335/pexels-photo-3165335.jpeg?auto=compress&cs=tinysrgb&w=400'} alt={s.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" /></div><span className={`absolute top-3 right-3 px-2 py-1 text-xs font-bold text-white ${pc[p]} rounded uppercase`}>{s.platform}</span><div className="p-4"><h3 className="font-display text-sm font-bold text-white group-hover:text-orange transition-colors line-clamp-2">{s.title}</h3><p className="text-xs text-gray-500 mt-1">{s.channel_name}</p><p className="text-xs text-gray-600 mt-1">{s.views.toLocaleString()} {t.home.views}</p></div></a>)}</div></section>
          )})}
          {!featuredLoading && featuredStreams.length > 0 && <section className="mb-12"><h2 className="section-title gradient-text mb-6">⭐ {t.streaming.highlights}</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{featuredStreams.map((s) => <a key={s.id} href={s.channel_url} target="_blank" rel="noopener noreferrer" className="group relative overflow-hidden rounded-xl news-card border-orange/30 card-hover"><div className="aspect-video overflow-hidden"><img src={s.thumbnail || 'https://images.pexels.com/photos/3165335/pexels-photo-3165335.jpeg?auto=compress&cs=tinysrgb&w=400'} alt={s.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" /></div><span className="absolute top-3 left-3 px-2 py-1 text-xs font-bold text-white bg-orange rounded">⭐</span><div className="p-4"><h3 className="font-display text-sm font-bold text-white group-hover:text-orange transition-colors line-clamp-2">{s.title}</h3><p className="text-xs text-gray-500 mt-1">{s.channel_name}</p></div></a>)}</div></section>}
        </>
      )}
    </div>
  )
