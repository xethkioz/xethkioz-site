...
export default function FunPortal() {
  ...
  const curated = getCuratedExternalNews('community')
  const seen = new Set<string>()
  const articles = [...published, ...curated].filter((article) => !seen.has(article.slug) && Boolean(seen.add(article.slug))).slice(0, 9)

  useEffect(() => {
    let activeRequest = true
    void fetchPublishedNews('community').then((next) => { if (activeRequest) setPublished(next) }).catch(() => undefined)
    ...
    <>
      ...
      <section className="xk-page px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="xk-section-panel relative overflow-hidden rounded-[2rem] border border-yellow-300/45 bg-[linear-gradient(135deg,rgba(234,179,8,.16),rgba(139,92,246,.12),rgba(50,255,138,.08))] p-6 shadow-[0_0_42px_rgba(234,179,8,.22)] md:p-10">
            <SafeImage src="/news/memes/gourmet-ai-meme.svg" fallback="/images/articles/community.svg" alt="" className="absolute right-0 top-0 h-full w-full object-cover opacity-25 sm:w-1/2" />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,.94),rgba(0,0,0,.72)_58%,rgba(139,92,246,.12))]" />
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            {t.blocks.map((item) => (
              <button key={item.id} type="button" onClick={() => selectBlock(item.id)} aria-pressed={active.id === item.id} className={`xk-card relative overflow-hidden rounded-3xl border p-5 text-left transition hover:-translate-y-1 ${active.id === item.id ? 'border-[#32FF8A]/70 bg-[linear-gradient(135deg,rgba(50,255,138,.12),rgba(234,179,8,.1))]' : 'border-yellow-300/30 bg-black/55 hover:shadow-[0_0_22px_rgba(234,179,8,.22)]'}`}>
                <span className="absolute right-4 top-3 text-4xl opacity-35" aria-hidden="true">{item.id === 'memes' ? '😂' : item.id === 'clips' ? '🎬' : '👽'}</span>
          ...
          <section className="mt-8 rounded-3xl border border-yellow-300/25 bg-black/55 p-5 md:p-7">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {articles.map((article) => (
                <article key={article.slug} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] transition hover:-translate-y-1 hover:border-yellow-300/40">
                  <SafeImage src={article.cover_image_url} fallback="/news/memes/argentina-duendes-cuartos.svg" alt={article.cover_image_alt || article.title} className="aspect-video w-full object-cover transition duration-700 group-hover:scale-[1.035]" />
                  <div className="p-5">
