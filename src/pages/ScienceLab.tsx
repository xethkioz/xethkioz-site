    ...
    <div className="min-h-screen bg-[#02070d] text-sky-50">
      ...
      <header className="sticky top-0 z-40 border-b border-sky-300/20 bg-black/75 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <Link to="/" className="font-display text-sm font-black uppercase tracking-[0.22em] text-sky-300 hover:text-white">{t.v7.backCore}</Link>
          <span className="rounded-full border border-sky-300/30 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-sky-300">{portal.label}</span>
        </div>
      </header>
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        <section className="rounded-[2rem] border border-sky-300/25 bg-black/55 p-7 shadow-[0_0_70px_rgba(14,165,233,.14)]">
          <p className="section-eyebrow text-sky-300">{portal.eyebrow}</p>
          <h1 className="mt-4 font-display text-4xl font-black md:text-6xl">{portal.heading}</h1>
          <p className="mt-5 max-w-3xl text-sm leading-relaxed text-sky-100/75 md:text-base">{portal.description}</p>
        </section>
