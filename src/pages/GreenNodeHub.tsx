...
const labCards = {
  ...
  en: [
    { code: '>_', title: 'Safe terminal', text: 'Learn terminal and Linux concepts inside a simulation that never executes commands on your device.' },
    { code: 'TLS', title: 'HTTPS without myths', text: 'What TLS protects, what the padlock does not prove and why the domain still matters.' },
    { code: 'KEY', title: 'Passkeys and MFA', text: 'Phishing-resistant authentication, recovery and common mistakes without hype.' },
    { code: 'HASH', title: 'File integrity', text: 'SHA-256, signatures, official repositories and what each mechanism can actually prove.' },
  ],
} as const
...
export default function GreenNodeHub() {
  ...
  return (
    <>
      <SEO
        title={es ? 'Green Node Protect · Seguridad, privacidad e Internet SOS' : 'Green Node Protect · Security, privacy and Internet SOS'}
        description={es
          ? 'Green Node Protect de XETHKIOZ: herramientas locales, guías de seguridad digital, privacidad, Linux, alertas verificadas y un Vault 13 separado del contenido factual.'
          : 'XETHKIOZ Green Node Protect: local-first tools, digital safety guidance, privacy, Linux, verified alerts and a separate Vault 13 for classified-style dossiers.'}
        url="/green-node"
        ...
        <div className="xk-gn2-shell">
          ...
          <section id="protect" className="xk-gn2-section" aria-labelledby="protect-title">
            ...
            <div className="xk-gn2-help-grid">
              {quickHelp[lang].map((item) => <Link key={item.code} to={item.href}><b>{item.code}</b><strong>{item.title}</strong><span>{item.text}</span><small>{es ? 'ABRIR GUÍAS →' : 'OPEN GUIDES →'}</small></Link>)}
            </div>
            <InternetSOS />
          </section>
          ...
          <section id="lab" className="xk-gn2-section" aria-labelledby="lab-title">
            ...
            <div className="xk-gn2-lab-grid">
              {labCards[lang].map((item) => <article key={item.code}><b>{item.code}</b><strong>{item.title}</strong><p>{item.text}</p></article>)}
            </div>
          </section>
