...
export default function Media() {
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{media.map((m) => <div key={m.id} className="group relative overflow-hidden rounded-xl news-card card-hover"><div className="aspect-[4/3] overflow-hidden"><SafeImage src={m.thumbnail || m.url} fallback="/images/articles/fallback.svg" alt={m.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" /></div>{m.is_featured && <span className="absolute top-3 left-3 px-2 py-1 text-xs font-bold text-white bg-orange rounded">⭐ {t.media.featured}</span>}<span className={`absolute top-3 right-3 px-2 py-1 text-xs font-bold text-white ${tb[m.type]} rounded uppercase`}>{m.type}</span><div className="p-4"><h3 className="font-display text-sm font-bold text-white group-hover:text-orange transition-colors">{m.title}</h3>{m.description && <p className="text-xs text-gray-500 mt-1 line-clamp-2">{m.description}</p>}</div></div>)}</div>}
    </div>
  )
}
