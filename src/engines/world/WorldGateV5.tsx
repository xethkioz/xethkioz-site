...
import { canEnterPortal, getPortalRegistry, type PortalTheme } from '../portal'
import { WispHUD } from '../wisp'
...
const copy = {
  es: {
    ...
    release: 'V5.1.0_GREEN_OVERRIDE',
    updateHeadline: 'Core Registry + Wisp Gateway activos',
    updateText: 'La Home queda ordenada por motores: Portal Registry, Wisp HUD, Design Tokens y señales dinámicas sin saturar la vista principal.',
    ...
    wispTitle: '// Entidad Asistente',
    portalTitle: '// Portales Estabilizados',
    greenOverride: 'OVERRIDE: INGRESO A LA MATRIX VERDE DE XETHKIOZ...',
    locked: 'Bloqueado por Wisp',
  },
  en: {
    ...
    systemCore: 'SYSTEM_CORE',
    release: 'V5.1.0_GREEN_OVERRIDE',
    updateHeadline: 'Core Registry + Wisp Gateway active',
    updateText: 'The Home is now engine-driven: Portal Registry, Wisp HUD, Design Tokens and dynamic signals without saturating the main view.',
    ...
    wispTitle: '// Assistant Entity',
    portalTitle: '// Stabilized Portals',
    greenOverride: 'OVERRIDE: ENTERING XETHKIOZ GREEN MATRIX...',
    locked: 'Locked by Wisp',
  },
} as const

export default function WorldGateV5() {
  const { lang, t } = useLang()
  const { state, mood, energy, registerEvent } = useWisp()
  const { xp, level } = useProfileProgress()
  const navigate = useNavigate()
  const ui = copy[lang]
  const greenMode = state === 'GREEN_MODE'

  useEffect(() => {
    if (!greenMode) return undefined
    const timer = window.setTimeout(() => navigate('/green-node'), 1450)
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-fusionBg text-gray-300 selection:bg-fusionAccent-tech-primary/30">
      <GreenModeOverlay active={greenMode} label={ui.greenOverride} />
        ...
        <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
          <div className="flex flex-col gap-8 lg:col-span-8">
            <section className="flex flex-col gap-4" aria-labelledby="world-gates-heading">
              ...
              <p className="font-mono text-xs uppercase tracking-[0.18em] text-gray-500">{ui.portalTitle}</p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {portalCards.map((portal) => {
                  const portalDisabled = !portal.canEnter
                  return (
                    <Link
                      key={portal.id}
                      to={portalDisabled ? '#' : portal.route}
                      aria-disabled={portalDisabled}
                      aria-label={`${t.v7.enter} ${portal.title}`}
                      onClick={(event) => {
                        if (portalDisabled) event.preventDefault()
                      }}
                      onMouseEnter={() => registerEvent('portal-hover', `home-v5:${portal.id}`, portal.route)}
                      className={`panel-cyber group flex h-36 flex-col justify-between p-4 transition duration-300 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 ${toneClass[portal.theme]} ${portalDisabled ? 'cursor-not-allowed opacity-60 grayscale' : ''}`}
                    >
                      ...
                      <span className="space-y-1">
                        ...
                        <small className="line-clamp-2 text-xs text-gray-500">{portalDisabled ? ui.locked : portal.subtitle}</small>
                      </span>
                    </Link>
                  )
                })}
          ...
          <aside className="flex flex-col gap-8 lg:col-span-4" aria-label="World Gate side systems">
            <div className="flex flex-col gap-4">
              <h2 className="font-mono text-xs uppercase tracking-[0.22em] text-gray-500">{ui.wispTitle}</h2>
              <WispHUD />
            </div>
