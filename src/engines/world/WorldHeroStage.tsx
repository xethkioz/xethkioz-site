...
export function WorldHeroStage() {
  ...
  return (
    <section
      className="panel-cyber relative z-10 flex flex-col items-center gap-8 overflow-hidden border border-fusionSurface-muted bg-gradient-to-b from-fusionSurface via-fusionBg to-fusionBg p-6 md:p-8"
      aria-labelledby="world-gate-title"
    >
      ...
      <div className="absolute left-4 top-3 font-mono text-[9px] uppercase tracking-[0.28em] text-gray-600">
        WORLD_ENGINE // STAGE_CORE
      </div>

      <div className="relative z-10 mt-6 flex select-none flex-col items-center gap-2 text-center">
        <h1
          id="world-gate-title"
          className="bg-gradient-to-r from-fusionAccent-gaming via-fusionAccent-textPrimary to-fusionAccent-fun bg-clip-text text-6xl font-black uppercase tracking-[0.08em] text-transparent drop-shadow-[0_0_25px_rgba(138,46,255,0.4)] md:text-8xl"
        >
          XETHKIOZ
        </h1>
        <p className="max-w-3xl font-mono text-xs uppercase tracking-[0.22em] text-fusionAccent-fun">
          {lang === 'es'
      ...
      <div className="relative z-10 flex w-full max-w-5xl flex-col items-center justify-center gap-8 py-2 lg:flex-row">
        ...
        <div className="flex w-full flex-1 flex-col gap-4">
          <div className="flex min-h-[76px] flex-col justify-center rounded-xl border border-fusionSurface-muted bg-fusionBg/60 p-4 backdrop-blur-sm">
            <p className="text-sm leading-relaxed text-fusionAccent-textSecondary transition-all" aria-live="polite">
          ...
          <div className="flex flex-wrap gap-2" role="group" aria-label={lang === 'es' ? 'Acciones del avatar' : 'Avatar actions'}>
            {roleActions.map((action) => (
              <button
                ...
                className={`rounded-lg border px-3 py-2 font-mono text-xs uppercase tracking-[0.08em] transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fusionAccent-gaming ${
                  activeAction.id === action.id
                    ? 'border-fusionAccent-gaming bg-fusionAccent-gaming/20 text-white shadow-[0_0_15px_rgba(138,46,255,0.3)]'
                    : 'border-fusionSurface-muted bg-fusionSurface text-gray-400 hover:border-gray-500 hover:text-white'
                }`}
