...
# Natural edge transitions, varied sizes, dense route blockers, persistence and QA jumps.
...
const MANIFEST_PATH: String = "res://data/regions/izrdralar_map_chain_01_32.json"
const PROGRESS_PATH: String = "user://izrdralar_continent_test.json"
const CHUNK_PIXELS: int = 512
const EDGE_MARGIN: float = 34.0
const EDGE_TRIGGER_RADIUS: float = 58.0
const SPAWN_INSET: float = 118.0
const TRANSITION_FADE: float = 0.18
...
var entry_path_position: Vector2 = Vector2.ZERO
var exit_path_position: Vector2 = Vector2.ZERO
var _transition_locked: bool = false
var _transition_grace: float = 0.75
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if not key_event.pressed or key_event.echo:
		return
	if key_event.physical_keycode == KEY_PAGEDOWN and current_map_id < 32:
		_begin_transition(current_map_id + 1, "forward", true)
	elif key_event.physical_keycode == KEY_PAGEUP and current_map_id > 1:
		_begin_transition(current_map_id - 1, "backward", true)
	elif key_event.physical_keycode == KEY_HOME and current_map_id != 1:
		_begin_transition(1, "forward", true)
	elif key_event.physical_keycode == KEY_END and current_map_id != 32:
		_begin_transition(32, "forward", true)

func _requested_map_id() -> int:
	var env_id: String = OS.get_environment("XETHKIOZ_MAP_ID")
	...
	var saved_id: int = _load_saved_map_id()
	if saved_id > 0:
...
func _load_saved_map_id() -> int:
	...
	if parsed is Dictionary:
		return int((parsed as Dictionary).get("map_id", 0))
	return 0
...
func _map_by_id(map_id: int) -> Dictionary:
	...
	for item in maps:
		if item is Dictionary:
			var map_data: Dictionary = item as Dictionary
			if int(map_data.get("id", 0)) == map_id:
				return map_data
	return {}

func _build_current_map() -> void:
	var size_data: Array = current_map.get("s", [2, 2])
	var width_chunks: int = maxi(1, int(size_data[0]))
	var height_chunks: int = maxi(1, int(size_data[1]))
...
func _build_chunks(width_chunks: int, height_chunks: int) -> void:
	var seed_base: int = 21500000 + current_map_id * 7919
	for y in range(height_chunks):
		for x in range(width_chunks):
			...
			add_child(chunk)
			move_child(chunk, 0)
			chunk.call("configure", coord, _chunk_data(coord, width_chunks, height_chunks), seed_base)
...
func _enable_outer_route(exits: Dictionary, coord: Vector2i, width_chunks: int, height_chunks: int, edge: String) -> void:
	match edge:
		"west":
			if coord.x == 0 and coord.y == (height_chunks >> 1): exits["w"] = true
		"east":
			if coord.x == width_chunks - 1 and coord.y == (height_chunks >> 1): exits["e"] = true
		"north":
			if coord.y == 0 and coord.x == (width_chunks >> 1): exits["n"] = true
		"south":
			if coord.y == height_chunks - 1 and coord.x == (width_chunks >> 1): exits["s"] = true

func _biome_for_chunk(coord: Vector2i) -> String:
	var biomes: Array = current_map.get("bi", ["forest"])
	if biomes.is_empty():
		return "forest"
	var index: int = posmod(coord.x + coord.y * 2 + current_map_id, biomes.size())
	return str(biomes[index])

func _is_vertical_route() -> bool:
	var entry_edge: String = str(current_map.get("e", "west"))
	var exit_edge: String = str(current_map.get("x", "east"))
	return entry_edge in ["north", "south"] and exit_edge in ["north", "south"]

func _build_route_sculpture() -> void:
	var profile: String = str(current_map.get("lp", "route"))
	var map_type: String = str(current_map.get("t", "normal"))
	if map_type == "boss":
...
func _route_color() -> Color:
	var biomes: Array = current_map.get("bi", ["forest"])
	var first: String = str(biomes[0]) if not biomes.is_empty() else "forest"
	match first:
		"river", "lake": return Color(0.08, 0.20, 0.24, 0.72)
		"ruins", "sanctuary": return Color(0.17, 0.16, 0.19, 0.76)
		"boss": return Color(0.20, 0.10, 0.12, 0.70)
		"refuge": return Color(0.10, 0.20, 0.15, 0.68)
		_: return Color(0.06, 0.17, 0.10, 0.75)

func _build_horizontal_switchbacks() -> void:
	var count: int = 1 if world_size.x <= 1200.0 else 2
	var nav: float = float(current_map.get("nav", 0.50))
	var gap_size: float = lerpf(180.0, 300.0, nav)
	for i in range(count):
		var fraction: float = float(i + 1) / float(count + 1)
		var x_pos: float = world_size.x * fraction
		var gap_y: float = world_size.y * (0.34 if (i + current_map_id) % 2 == 0 else 0.68)
		_add_vertical_band(x_pos, gap_y, gap_size, 72.0)

func _build_vertical_switchbacks() -> void:
	var count: int = 1 if world_size.y <= 1200.0 else 2
	var nav: float = float(current_map.get("nav", 0.50))
	var gap_size: float = lerpf(180.0, 300.0, nav)
	for i in range(count):
		var fraction: float = float(i + 1) / float(count + 1)
		var y_pos: float = world_size.y * fraction
		var gap_x: float = world_size.x * (0.34 if (i + current_map_id) % 2 == 0 else 0.68)
		_add_horizontal_band(y_pos, gap_x, gap_size, 72.0)

func _build_dungeon_bands() -> void:
	var fractions: Array[float] = [0.28, 0.52, 0.76]
	if _is_vertical_route():
		for index in range(fractions.size()):
			var fraction_value: float = fractions[index]
			var gap_x: float = world_size.x * (0.35 if index % 2 == 0 else 0.65)
			_add_horizontal_band(world_size.y * fraction_value, gap_x, 185.0, 82.0)
	else:
		for index in range(fractions.size()):
			var fraction_value: float = fractions[index]
			var gap_y: float = world_size.y * (0.34 if index % 2 == 0 else 0.68)
			_add_vertical_band(world_size.x * fraction_value, gap_y, 185.0, 82.0)

func _build_corner_mass() -> void:
	var size: Vector2 = Vector2(minf(210.0, world_size.x * 0.22), minf(180.0, world_size.y * 0.22))
	_add_route_blocker(Rect2(Vector2(60, 60), size))
	_add_route_blocker(Rect2(Vector2(world_size.x - size.x - 60, 60), size))
	_add_route_blocker(Rect2(Vector2(60, world_size.y - size.y - 60), size))
	_add_route_blocker(Rect2(Vector2(world_size.x - size.x - 60, world_size.y - size.y - 60), size))

func _build_hub_mass() -> void:
	_add_route_blocker(Rect2(Vector2(world_size.x * 0.42, world_size.y * 0.16), Vector2(world_size.x * 0.16, world_size.y * 0.25)))
	_add_route_blocker(Rect2(Vector2(world_size.x * 0.42, world_size.y * 0.60), Vector2(world_size.x * 0.16, world_size.y * 0.24)))
	_add_route_blocker(Rect2(Vector2(world_size.x * 0.18, world_size.y * 0.38), Vector2(world_size.x * 0.15, world_size.y * 0.24)))
	_add_route_blocker(Rect2(Vector2(world_size.x * 0.68, world_size.y * 0.38), Vector2(world_size.x * 0.15, world_size.y * 0.24)))

func _build_boss_cover(profile: String) -> void:
	if profile == "boss_final_large":
		var points: Array[Vector2] = [Vector2(0.27, 0.30), Vector2(0.73, 0.30), Vector2(0.27, 0.70), Vector2(0.73, 0.70)]
		for uv in points:
			_add_route_blocker(Rect2(Vector2(world_size.x * uv.x - 45.0, world_size.y * uv.y - 45.0), Vector2(90, 90)))
	elif profile == "boss_frontier_wide":
		_add_route_blocker(Rect2(Vector2(world_size.x * 0.46, world_size.y * 0.20), Vector2(80, world_size.y * 0.18)))
		_add_route_blocker(Rect2(Vector2(world_size.x * 0.46, world_size.y * 0.62), Vector2(80, world_size.y * 0.18)))
	else:
		_add_route_blocker(Rect2(Vector2(world_size.x * 0.18, world_size.y * 0.22), Vector2(72, 72)))
		_add_route_blocker(Rect2(Vector2(world_size.x * 0.74, world_size.y * 0.66), Vector2(72, 72)))

func _add_vertical_band(x_pos: float, gap_center_y: float, gap_size: float, thickness: float) -> void:
	var top_h: float = maxf(0.0, gap_center_y - gap_size * 0.5)
	var bottom_y: float = minf(world_size.y, gap_center_y + gap_size * 0.5)
	if top_h > 24.0:
		_add_route_blocker(Rect2(Vector2(x_pos - thickness * 0.5, 0), Vector2(thickness, top_h)))
	if world_size.y - bottom_y > 24.0:
		_add_route_blocker(Rect2(Vector2(x_pos - thickness * 0.5, bottom_y), Vector2(thickness, world_size.y - bottom_y)))

func _add_horizontal_band(y_pos: float, gap_center_x: float, gap_size: float, thickness: float) -> void:
	var left_w: float = maxf(0.0, gap_center_x - gap_size * 0.5)
	var right_x: float = minf(world_size.x, gap_center_x + gap_size * 0.5)
	if left_w > 24.0:
		_add_route_blocker(Rect2(Vector2(0, y_pos - thickness * 0.5), Vector2(left_w, thickness)))
	if world_size.x - right_x > 24.0:
		_add_route_blocker(Rect2(Vector2(right_x, y_pos - thickness * 0.5), Vector2(world_size.x - right_x, thickness)))
...
func _edge_position(edge: String) -> Vector2:
	match edge:
		...
		_: return Vector2(EDGE_MARGIN, world_size.y * 0.5)
...
func _build_zone_markers() -> void:
	...
	var vertical: bool = _is_vertical_route()
	...
	for index in range(zones.size()):
		var kind: String = str(zones[index])
		var uv: Vector2 = anchors[index % anchors.size()]
		var pos: Vector2 = Vector2(uv.x * world_size.x, uv.y * world_size.y)
...
func _add_zone_marker(index: int, pos: Vector2, label_text: String, kind: String) -> void:
	...
	var zone_color: Color = _zone_color(kind)
	plate.color = Color(zone_color.r, zone_color.g, zone_color.b, 0.17)
...
func _spawn_player() -> void:
	...
	var entry_mode: String = str(GameState.get_meta("izrdralar_entry", "forward"))
...
func _size_class(size_data: Array) -> String:
	var width_count: int = int(size_data[0])
	var height_count: int = int(size_data[1])
	if width_count == 3 and height_count == 3: return "FINAL LARGE"
	if width_count * height_count >= 8: return "DOBLE"
	if width_count * height_count >= 6: return "MEDIO"
	return "COMPACTO"

func _map_title_text() -> String:
	var size_data: Array = current_map.get("s", [2, 2])
	var px_w: int = int(size_data[0]) * CHUNK_PIXELS
	var px_h: int = int(size_data[1]) * CHUNK_PIXELS
	return "IZR-M%02d · %s · BLOQUE %s\n%s · %dx%d · DIFICULTAD %d" % [current_map_id, str(current_map.get("n", "")), str(current_map.get("b", "")), _size_class(size_data), px_w, px_h, int(current_map.get("d", 1))]

func _update_zone_hint() -> void:
	var nearest_zone: String = ""
...
func _update_edge_hint_and_transition() -> void:
	...
	var exit_edge: String = str(current_map.get("x", "east"))
	if current_map_id > 1:
		var prev_distance: float = player.global_position.distance_to(entry_path_position)
		...
		if _transition_grace <= 0.0 and prev_distance <= EDGE_TRIGGER_RADIUS and player.velocity.dot(_outward_vector(entry_edge)) > 4.0:
			...
			return
	var next_distance: float = player.global_position.distance_to(exit_path_position)
	if current_map_id < 32:
...
func _begin_transition(target_id: int, entry_mode: String, debug_jump: bool) -> void:
	...
	EventBus.toast_requested.emit(("TEST · salto a IZR-M%02d" if debug_jump else "Camino · IZR-M%02d") % target_id)
...
func _ensure_inputs() -> void:
	...
	for action_name in actions.keys():
		...
		var keycodes: Array = actions[action_name]
		for keycode in keycodes:
			var exists: bool = false
			for current in InputMap.action_get_events(action):
				if current is InputEventKey and (current as InputEventKey).physical_keycode == int(keycode):
					exists = true
			if not exists:
				var input_key: InputEventKey = InputEventKey.new()
				input_key.physical_keycode = int(keycode)
