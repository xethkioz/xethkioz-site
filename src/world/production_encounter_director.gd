...
func _on_stage_changed(stage_id: String) -> void:
	...
	match stage_id:
		...
		"refuge_after_boss":
			_spawned_stages[stage_id] = true
			EventBus.toast_requested.emit("El bosque se calmó · regresá al Refugio de Elida")
		"mentor_choice":
			_spawned_stages[stage_id] = true
		"fermin_training":
			_spawned_stages[stage_id] = true
			EventBus.toast_requested.emit("La prueba de Fermín te espera dentro del Refugio")
