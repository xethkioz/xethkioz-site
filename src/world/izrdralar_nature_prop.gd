...
func _draw_tree_canopy(p: Dictionary) -> void:
	...
	var centers := [Vector2(-17,-43),Vector2(0,-53),Vector2(18,-44),Vector2(-7,-35),Vector2(10,-31)]
	for i in range(centers.size()):
		var c: Vector2 = centers[i] * s + Vector2(sway * (0.55 + i*0.07),0)
		var radius := (16.0 + float((variant_seed + i) % 4) * 2.1) * s
...
func _draw_tree_birch(p: Dictionary) -> void:
	var s := prop_scale
	var sway := sin(_phase+0.8) * 1.35*s
	...
	var centers := [Vector2(-9,-48),Vector2(5,-55),Vector2(13,-44),Vector2(-14,-36)]
	for i in range(centers.size()):
		var c := centers[i]*s + Vector2(sway,0)
		var r := (12.0 + float(i%2)*2.0)*s
...
func _draw_bush(p: Dictionary, flowers: bool) -> void:
	var s := prop_scale
	var sway := sin(_phase) * 0.65*s
	_draw_shadow(Vector2(0,3*s), Vector2(17*s,5*s))
	var centers := [Vector2(-12,-5),Vector2(0,-10),Vector2(12,-4),Vector2(-5,1),Vector2(7,2)]
	for i in range(centers.size()):
		var c := centers[i]*s + Vector2(sway*(0.4+i*0.08),0)
		var r := (8.5+float((variant_seed+i)%3))*s
		draw_circle(c,r+1*s,p.leaf_dark)
		draw_circle(c+Vector2(-1*s,-2*s),r,p.leaf)
		draw_circle(c+Vector2(-3*s,-4*s),r*0.34,p.leaf_light)
	if flowers:
		for i in range(4):
			var x := (-10.0 + float(i)*7.0 + float((variant_seed+i)%3))*s
			var y := (-8.0 + float((variant_seed+i*3)%7))*s
...
func _draw_stump(p: Dictionary) -> void:
	...
	draw_ellipse(Vector2(0,-10*s),Vector2(10*s,4*s),p.wood_dark)
	draw_ellipse(Vector2(0,-11*s),Vector2(8*s,3*s),Color("a47b54"))
...
func _draw_rock(p: Dictionary, large: bool) -> void:
	var s := prop_scale*(1.35 if large else 1.0)
...
func _draw_shadow(center: Vector2, radii: Vector2) -> void:
	draw_ellipse(center,radii,Color(0,0,0,0.22))

func draw_ellipse(center: Vector2, radii: Vector2, color_value: Color) -> void:
