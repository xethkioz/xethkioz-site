...
test.describe('orden y navegación de secciones', () => {
  test('Inicio prioriza World of Xethkioz y conserva el ecosistema', async ({ page }) => {
    ...
    await expect(ecosystem.getByRole('link', { name: 'JUEGO' })).toHaveAttribute('href', '/world-of-xethkioz')
    await expect(ecosystem.getByRole('link', { name: 'BIBLIOTECA DE JUEGOS' })).toHaveAttribute('href', '/gaming')
    ...
    await expect(ecosystem.getByRole('link', { name: 'NEXUS CITY' })).toHaveAttribute('href', '/nexus-city')
    await expect(ecosystem.getByRole('link', { name: 'CREACIÓN WEB' })).toHaveAttribute('href', '/creacion-web')
