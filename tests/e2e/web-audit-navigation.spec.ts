...
test('WEB-06: mobile menu closes with Escape and keeps its links above the hero', async ({ page }) => {
  ...
  await essentials(page)
  await openMenu(page)
  await expect(page.locator('.xkf-mobile')).toHaveAttribute('aria-expanded', 'true')
  ...
  await page.locator('.xkf-mobile-panel a').first().focus()
    ...
    for (const route of ['/', '/world-of-xethkioz', '/en', '/en/world-of-xethkioz']) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        return {
          ...
          minFont: Math.min(...controls.map(el => parseFloat(getComputedStyle(el).fontSize))),
...
test('WEB-06: complete fantasy shell passes automated accessibility checks', async ({ page }) => {
  await page.goto('/world-of-xethkioz')
  await essentials(page)
